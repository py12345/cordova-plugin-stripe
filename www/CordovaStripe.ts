...
export namespace CordovaStripe {
  ...
  export class Plugin {
    ...
    static getGooglePayToken(options: GooglePayOptions, success: (token: TokenResponse) => void, error: ErrorCallback = NOOP) {
      exec(success, error, 'CordovaStripe', 'getGooglePayToken', [options.amount, options.currencyCode]);
    }
